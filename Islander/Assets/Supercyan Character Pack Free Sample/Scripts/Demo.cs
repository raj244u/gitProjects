...
public class Demo : MonoBehaviour {
    ...
    private void Start()
    {
        PlayerAnimators = FindObjectsOfType<Animator>();
    }
    ...
    private void Update()
    {
        ...
        {
            m_cameraLogic.PreviousAxeList();
        }
        if (Input.GetKeyDown(KeyCode.E))
        {
            m_cameraLogic.NextAxeList();
        }
    }
    ...
    private void OnGUI()
    {
        ...
        for(int i = 0; i < m_animations.Length; i++)
        {
            ...
            {
                for(int j = 0; j < PlayerAnimators.Length; j++)
                {
                    PlayerAnimators[j].SetTrigger(m_animations[i]);
                }
            }
